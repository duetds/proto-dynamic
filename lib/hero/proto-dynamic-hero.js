    ...
    render() {
      ...
      ${nodes.map(node => {
            ...
            if (node.nodeType === "paragraph") {
                renderedContent = node.content?.map(n => n.nodeType === "text" ? renderRichText(n) : unsafeHTML(renderRichText(n)));
            }
            else if (node.nodeType === "embedded-entry-block" && node.data?.target?.fields) {
                return unsafeHTML(renderRichText(node));
            }
            return html `
          <duet-grid
            grid-template=${this.isParentLarge && this.isLargeScreen ? "sidebar-right" : nothing}
          >
            <duet-paragraph>${renderedContent}</duet-paragraph>
          </duet-grid>
        `;
        })}
